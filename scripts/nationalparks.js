...
function waysToSearchAppend() {
    let WaysOption = document.getElementById("WaysToSearch");
    let waysToSearchLenght = waysToSearch.length;
    let defaultOption = document.createElement("Option");
    defaultOption.text = "Select one";
    WaysOption.appendChild(defaultOption);
    for (let k = 0; k < waysToSearchLenght; k++) {
        let theOption = new Option(waysToSearch[k])

        WaysOption.appendChild(theOption)
    };
};


// this is the option that makes the other select show or unshow when it selected 
function onWaysOptionsChange() {
    const waysOptionValue = document.getElementById("WaysToSearch").value;

    if (waysOptionValue == "Location") {
        let parkType = document.getElementById("parkType")
    ...
    } else if (waysOptionValue == "Search By Type") {
        let parkType = document.getElementById("parkType")
        parkType.style.display = ""
...
// this is anaother function that programatically takes the array into an option 
function selectSearchByLocationDropDown() {
    let location = document.getElementById("location");
    let locationsArrayLenght = locationsArray.length;

    for (let k = 0; k < locationsArrayLenght; k++) {
        let theOption = new Option(locationsArray[k])

        location.appendChild(theOption)
    }

};
// this is anaother function that programatically takes the array into an option 
function selectSearchByParkType() {
    let parkType = document.getElementById("parkType")
    let parkTypesArrayLength = parkTypesArray.length
    for (let k = 0; k < parkTypesArrayLength; k++) {
        let theOption = new Option(parkTypesArray[k])

        parkType.appendChild(theOption)
...
//this function receives multiple parks and then returns only the parks with a matching location.
function getParksByLocation(parks, State) {
    let results = [];
    let nationalParksArrayLength = parks.length;
    for (let k = 0; k < nationalParksArrayLength; k++) {
        if (parks[k].State == State) {
            results.push(parks[k]);
...
//searches parks and returns only those parks of this type
function getParksByType(parks, type) {
    let results = []
    let nationalParksArrayLength = parks.length;
    for (let k = 0; k < nationalParksArrayLength; k++) {
        if (parks[k].LocationName.includes(type)) {
            results.push(parks[k])
    ...
    //this is returning multiple parks, one for each of the input parks that are this type.
    return results;

};


//this function receives multiple parks, for each park it should create a new row in the output table 
...
function searchBtnOnClick() {
    ...
    let waysToSearch= document.getElementById("WaysToSearch")
    let waysToSearchValue = waysToSearch.value

    let nationalParksArrayLength = nationalParksArray.length;


    let filteredParks;
    if (waysToSearchValue == "Location"){
